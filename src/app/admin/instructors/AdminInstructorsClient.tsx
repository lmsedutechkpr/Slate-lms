...
import { GraduationCap, BookOpen, CheckCircle, Clock, XCircle, Search, Loader2 } from 'lucide-react';
...
export default function AdminInstructorsClient({ initialInstructors }: AdminInstructorsClientProps) {
  ...
  const pendingCount = instructors.filter((i: any) => i.approval_status === 'pending').length;
  const approvedCount = instructors.filter((i: any) => i.approval_status === 'approved').length;
  const rejectedCount = instructors.filter((i: any) => i.approval_status === 'rejected').length;

  const filteredInstructors = useMemo(() => {
    let result = filter === 'all' ? instructors : instructors.filter((i: any) => i.approval_status === filter);
    if (search) {
  ...
  const handleApprove = async (id: string) => {
    ...
    if (result.success) {
      ...
      setInstructors(prev => prev.map(inst => inst.id === id ? { ...inst, approval_status: 'approved' } : inst));
    } else {
  ...
  const handleReject = async () => {
    ...
    const result = await rejectInstructor(rejectingId, rejectionReason);
    setLoadingAction(null);
    if (result.success) {
      toast.success('Instructor rejected');
      // local state update NO RELOAD
      setInstructors(prev => prev.map(inst => inst.id === rejectingId ? { ...inst, approval_status: 'rejected' } : inst));
      setRejectingId(null);
      setRejectionReason('');
    } else {
    ...
    <div className="min-h-full bg-gray-50 relative">
      ...
      {rejectingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-hidden">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 m-4 shadow-xl">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Reject Instructor</h3>
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-5">
        ...
        <p className="text-gray-400 text-sm mt-1">
          {approvedCount} approved · {pendingCount} pending · {instructors.length} total
        </p>
      </div>
            ...
            <>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredInstructors.map((instructor: any) => {
                      const conf = APPROVAL_CONFIG[instructor.approval_status] || APPROVAL_CONFIG.pending;
                      const StatusIcon = conf.icon;
                      const isPending = instructor.approval_status === 'pending';
                      return (
                        <tr
                          key={instructor.id}
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0 text-gray-600 text-sm font-bold overflow-hidden">
                                {instructor.avatar_url ? (
                                  <img src={instructor.avatar_url} alt={instructor.full_name} className="w-10 h-10 object-cover" />
                                ) : (
                                  instructor.full_name?.[0]?.toUpperCase() || '?'
                                )}
                              </div>
                              <div className="min-w-0 flex-1">
                                <p className="text-sm font-semibold text-gray-900 truncate" title={instructor.full_name}>
                                  {instructor.full_name || 'Unknown'}
                                </p>
                                <p className="text-xs text-gray-500 truncate" title={instructor.email}>{instructor.email}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            ...
                            {instructor.pendingCourses > 0 && (
                              <p className="text-xs text-amber-600 mt-0.5">{instructor.pendingCourses} pending review</p>
                            )}
                          </td>
                          <td className="px-6 py-4">
                            <span className="text-xs text-gray-500">
                              {new Date(instructor.created_at).toLocaleDateString('en-IN')}
                            </span>
                          </td>
                              ...
                              {isPending && (
                                <>
                                  <button
                                    ...
                                    disabled={loadingAction === `approve-${instructor.id}`}
                                  ...
                                  >
                                    {loadingAction === `approve-${instructor.id}` ? (
                                  ...
                                  <button
                                    onClick={() => setRejectingId(instructor.id)}
              ...
              <div className="md:hidden space-y-3 p-4">
                {filteredInstructors.map((instructor: any) => {
                  ...
                  return (
                    <div key={instructor.id} className={`bg-white rounded-xl p-4 border ${isPending ? 'border-amber-300 bg-amber-50/30' : 'border-gray-100'}`}>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs mb-4">
                        <div className="text-gray-500">
                          <p className="mb-0.5">Courses</p>
                          <p className="font-semibold text-gray-900">{instructor.publishedCourses}/{instructor.totalCourses}</p>
                        </div>
                        <div className="text-gray-500 text-right">
                          <p className="mb-0.5">Joined</p>
                          <p className="font-semibold text-gray-900">{new Date(instructor.created_at).toLocaleDateString('en-IN')}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                        {isPending && (
                          <>
                            ...
                            >
                              {loadingAction === `approve-${instructor.id}` ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle size={12} />}
                              Approve
                            </button>
                            <button
                              onClick={() => setRejectingId(instructor.id)}
                        ...
                        <Link href={`/admin/instructors/${instructor.id}`} className={isPending ? '' : 'w-full'}>
