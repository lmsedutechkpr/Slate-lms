...
import { getAdminInstructorDetail } from '@/lib/actions/admin';
import InstructorApprovalActions from '@/components/admin/InstructorApprovalActions';
import {
  ArrowLeft,
  BookOpen,
  Users,
...
export default async function AdminInstructorDetailPage({
  ...
  const data = await getAdminInstructorDetail(id);

  if (!data) notFound();

  const { profile, courses, totalStudents, totalRevenue } = data;
  const approvalConf =
    ...
    <div className="min-h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-5">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
          ...
          <span>›</span>
          <Link href="/admin/instructors" className="hover:text-gray-900 transition-colors">Instructors</Link>
          <span>›</span>
        ...
        >
          <ArrowLeft size={13} /> Back to instructors
        </Link>
      </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-50">
                  {courses.map((course: any) => {
                    const conf = STATUS_CONFIG[course.status] || STATUS_CONFIG.draft;
                    return (
                      <Link
                        key={course.id}
