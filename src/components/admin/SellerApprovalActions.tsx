...
import { approveSeller, rejectSeller, suspendSeller } from '@/lib/actions/admin';
import { toast } from 'sonner';

interface SellerApprovalActionsProps {
  sellerId: string;
  status: string;
}

export default function SellerApprovalActions({ sellerId, status }: SellerApprovalActionsProps) {
  ...
  const handleApprove = async () => {
    ...
    setLoading(null);
    if (result.success) {
      toast.success('Seller approved! They can now submit products.');
      router.refresh();
    } else {
  ...
  const handleReject = async () => {
    ...
    setLoading('reject');
    const result = await rejectSeller(sellerId, reason.trim());
    setLoading(null);
    if (result.success) {
  ...
  const handleSuspend = async () => {
    if (!confirm('Are you sure you want to suspend this seller account? They will lose access immediately.')) return;
    setLoading('suspend');
